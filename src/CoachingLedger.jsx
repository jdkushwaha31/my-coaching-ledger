...
import { LayoutGrid, Users, Wallet, Receipt, AlertCircle, Plus, Trash2, X, Check, ChevronDown } from "lucide-react";
...
export default function CoachingLedger() {
  ...
  useEffect(() => {
    (async () => {
      const [s, f, d] = await Promise.all([
        storageGet("students", []),
        storageGet("feeStructure", defaultFeeStructure()),
        storageGet("deposits", []),
      ]);
      setStudents(s.map(st => ({ status: "active", ...st }))); 
      setFeeStructure(f); setDeposits(d);
      setLoaded(true);
    })();
  }, []);

  const persist = useCallback(async (key, value) => {
    const ok = await window.storage.set(key, JSON.stringify(value), false).catch(() => null);
    if (!ok) setSaveError(true); else setSaveError(false);
  }, []);

  useEffect(() => { if (loaded) persist("students", students); }, [students, loaded, persist]);
  useEffect(() => { if (loaded) persist("feeStructure", feeStructure); }, [feeStructure, loaded, persist]);
  useEffect(() => { if (loaded) persist("deposits", deposits); }, [deposits, loaded, persist]);
  ...
  // Calculate total due per student
  ...
  function saveStudent(data) {
    if (data.id) {
      ...
      setStudents(prev => [...prev, { status: "active", ...data, id: uid() }]);
    }
  ...
  function removeStudent(id) {
    ...
    setDeposits(prev => prev.filter(d => d.studentId !== id));
  }
  function saveDeposit(data) {
    setDeposits(prev => [...prev, { ...data, id: uid() }]);
    setShowDepositForm(false);
  }
  function removeDeposit(id) { setDeposits(prev => prev.filter(d => d.id !== id)); }

  if (!loaded) {
    return <div className="min-h-screen flex items-center justify-center" style={{ background: "#FAF6EC", fontFamily: "'IBM Plex Mono', monospace", color: "#8A6420" }}>Opening the ledger…</div>;
  }
    ...
    <div className="min-h-screen flex" style={{ background: "#FAF6EC", fontFamily: "'Inter', sans-serif", color: "#26231D" }}>
      ...
      <aside className="w-56 shrink-0 flex flex-col" style={{ background: "#12312B" }}>
        ...
        <div className="px-4 py-4 text-[11px] leading-relaxed" style={{ color: "#6E9384", borderTop: "1px solid #24473F", fontFamily: "'IBM Plex Mono', monospace" }}>
          ...
          {saveError ? <span style={{ color: "#E0A05C" }}>sync error — retrying</span> : "saved locally"}
        </div>
      </aside>
      ...
      <main className="flex-1 px-8 py-7 max-w-6xl">
        ...
        {tab === "structure" && (
          <StructureTab feeStructure={feeStructure} setFeeStructure={setFeeStructure} />
        )}
        {tab === "deposits" && (
...
function StructureTab({ feeStructure, setFeeStructure }) {
  function update(cls, count, val) {
    setFeeStructure(prev => ({ ...prev, [cls]: { ...prev[cls], [count]: Number(val) || 0 } }));
  }
  return (
    <div>
      <SectionHeader eyebrow="Package pricing" title="Fee Structure" />
      <div className="text-sm text-[#6E6650] mb-4">Monthly fee by class and number of batches chosen. Editable — changes apply to future due calculations.</div>
      <Card>
        <table className="w-full text-sm">
          <thead>
            <tr style={{ borderBottom: "1.5px solid #26231D" }}>
...
function DepositsTab({ deposits, students, onAdd, onRemove, expectedFeeFor, batchesForMonth }) {
